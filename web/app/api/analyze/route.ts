...
export async function POST(request: Request) {
  try {
    ...
    const improveScriptPath = path.join(
      projectRoot,
      "ml",
    ...
    if (!fs.existsSync(improveScriptPath)) {
      throw new Error(
        `Improvement analysis script not found at: ${improveScriptPath}`,
      );
    }
    ...
    const runVideoAnalysis = (videoPath: string, outputPath: string) => {
      return new Promise((resolve, reject) => {
        const pythonProcess = spawn(
          pythonPath,
          [analyzeScriptPath, videoPath, outputPath],
          {
            cwd: projectRoot,
          },
        );
    ...
    const publicImprovementOutput = path.join(
      analysisDir,
    ...
    if (!fs.existsSync(improvementOutput)) {
      throw new Error(
        `Improvement analysis output not found at: ${improvementOutput}`,
      );
    }
    ...
    console.log("Copied analysis results to:", {
      publicBeforeOutput,
      publicAfterOutput,
      publicImprovementOutput,
    });
