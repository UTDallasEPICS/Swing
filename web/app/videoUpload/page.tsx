...
export default function VideoUpload() {
  ...
  const [activeUpload, setActiveUpload] = useState<"before" | "after" | null>(
    null,
  );
    ...
    for (const file of files) {
      ...
      // Otherwise, try to determine which video to set
        ...
        if (!beforeVideo) {
          ...
          setError(
            "Please select which video you want to upload (Before or After)",
          );
        }
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(
        `/results?before=${result.beforeResult}&after=${result.afterResult}&improvement=${result.improvementResult}`,
      );
    } catch (err) {
              ...
              <div className="bg-blue-50 p-4 rounded-lg flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-blue-700 font-medium">
                    Uploading {activeUpload === "before" ? "Before" : "After"}{" "}
                    Treatment video
                  </span>
                </div>
                <input
